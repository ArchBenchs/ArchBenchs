...
vector<node> compress(string in, int windowSize = 4096, int bufferSize = 15) {
    ...
    time_t s, t = 0;
    int length = in.size();
    vector<int> qwe(bufferSize + 1, 0);
    int pos = 0;

    while (pos < length) {
        for (int i = 0; i < bufferSize + 1; i++)qwe[i] = 0;
        int mm = max(0, pos - windowSize);
#pragma omp parallel for
        for (int i = mm; i < pos; i++) {
...
int main() {
#ifdef my_comp
    FILE* in = fopen("C:\\Users\\dzhak\\Documents\\aboba\\w a p.txt", "rb");
    ...
    ofstream fout("C:\\Users\\dzhak\\Documents\\aboba\\output.txt");
    //ofstream data_out("C:\\Users\\dzhak\\Documents\\aboba\\data.txt");
#endif

#ifndef my_comp
    FILE* in = fopen("1.txt", "rb");
    FILE* compressed = fopen("2.txt", "wb");
    FILE* decompressed = fopen("3.txt", "wb");
    FILE* io_check = fopen("4.txt", "wb");
    ...
    fout << mist1 << " " << mist2 << endl;
    //data_out << comp.size() << "\n";
    //for (int i = 0; i < comp.size(); i++)data_out << comp[i].len<<"\n";
    f_full = clock();
    cout << "Full time: " << f_full - s_full << endl;
    fout << f_full - s_full << endl;
    //data_out.close();
    fout.close();
}
